...
public class PruebaFireBase extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        myButton2.setOnClickListener(new View.OnClickListener(){
            ...
            /**Para ver los resultados de forma facil ir a vista "LogCat" y buscar
             * por el nombre del log de la prueba "PF_".
             * */
            ...
            public void onClick(View view){
                ...
                int pruebas = 5;
                int p_correcta = 0;
                String log = "PF_FuncFB";

                if(prubasInfoEventosOrganizador()){p_correcta++;}
                if(prubasValidarInvitacionActualizaReg()){p_correcta++;}
                if(prubasInfoIvitados()){p_correcta++;}
                if(prubasValidarInvitacionAsistido()){p_correcta++;}
                if(prubasDesvalidarInvitacionAsistido()){p_correcta++;}

                if (p_correcta == pruebas) {
                    Log.d(log + "-RESULTADO", "¡¡¡TODOS LOS TESTS CORRECTOS!!!");
                } else {
                    Log.i(log + "-RESULTADO", "¡¡¡FALLO EN LOS TEST!!!");
                }
            }
        });
     ...
     * por el nombre del log de la prueba "PF_infoEvenOrg".
     * Devuelve true si es corecto el test, false en caso contrario.
     */
    private boolean prubasInfoEventosOrganizador() {
     ...
     * ver los resultados de forma facil ir a vista "LogCat" y buscar
     * por el nombre del log de la prueba "PF_valInvActReg".
     * Devuelve true si es corecto el test, false en caso contrario.
     */
    private boolean prubasValidarInvitacionActualizaReg() {
     ...
     * En la función ya se crea el evento e invitado que se usan para validar, para
     * ver los resultados de forma facil ir a vista "LogCat" y buscar
     * por el nombre del log de la prueba "PF_infoIvitados".
     * Devuelve true si es corecto el test, false en caso contrario.
     */
    private boolean prubasInfoIvitados() {
    ...
    /**
     * Pruebas sobre la funcion ValidarInvitacionAsistido:
     * En la función ya se crea el evento e invitado que se usan para validar, para
     * ver los resultados de forma facil ir a vista "LogCat" y buscar
     * por el nombre del log de la prueba "PF_valInvActReg".
     * Devuelve true si es corecto el test, false en caso contrario.
     */
    private boolean prubasValidarInvitacionAsistido() {
        String log = "PF_valInvAsis";
        int pruebas = 4;//Numero de pruebas
        int p_correcta = 0;//Pruebas correctas
        Funciones_FireBase db = new Funciones_FireBase();
        ...
        Boolean exitosa;
        String organizador = "prueba@gmail.com";
        String nombre = "Prueba_ValInvAsis";
        String fecha = "2024-12-01";
        String hora = "00:00", ubicacion = "Centro Prueba";
        String id_evento = "";
        List<String> res = new ArrayList<>();
        ...
        List<Integer> respuesta = new ArrayList<>();
        exitosa = db.validarInvitacionAsistido("32", id_evento, respuesta);

        if(exitosa) {//Se ha establecido la consulta con exito
            Log.d(log + "-1", "CORRECTO---Conexion establecida");
            p_correcta++;
        } else {
            Log.d(log + "-1", "INCORRECTO---Fallo en la comunicacion con validarInvitacionAsistido");
        }
        ...
        if(exitosa) {//Se ha establecido la consulta con exito
            ...
            Log.d(log + "-2", "INCORRECTO---Fallo en la comunicacion con validarInvitacionAsistido");
        }
        ...
        if(exitosa) {//Se ha establecido la consulta con exito
            ...
            Log.d(log + "-3", "INCORRECTO---Fallo en la comunicacion con validarInvitacionAsistido");
        }
        ...
        if(exitosa) {//Se ha establecido la consulta con exito
            ...
            Log.d(log + "-4", "INCORRECTO---Fallo en la comunicacion con validarInvitacionAsistido");
        }
    ...
    /**
     * Pruebas sobre la funcion DesvalidarInvitacionAsistido:
     * En la función ya se crea el evento e invitado que se usan para validar, para
     * ver los resultados de forma facil ir a vista "LogCat" y buscar
     * por el nombre del log de la prueba "PF_DesValInvAsis".
     * Devuelve true si es corecto el test, false en caso contrario.
     */
    private boolean prubasDesvalidarInvitacionAsistido() {
        String log = "PF_DesValInvAsis";
        int pruebas = 4;//Numero de pruebas
        int p_correcta = 0;//Pruebas correctas
        Funciones_FireBase db = new Funciones_FireBase();
        ...
        Boolean exitosa;
        String organizador = "prueba@gmail.com";
        String nombre = "Prueba_DesValInvAsis";
        String fecha = "2024-12-01";
        String hora = "00:00", ubicacion = "Centro Prueba";
        String id_evento = "";
        List<String> res = new ArrayList<>();
        ...
        exitosa = db.DesvalidarInvitacionAsistido(id_invitado, id_evento);

        if(exitosa) {//Se ha establecido la consulta con exito
            Log.d(log + "-1", "CORRECTO---Conexion establecida");
            p_correcta++;
        } else {
            Log.d(log + "-1", "INCORRECTO---Fallo en la comunicacion con DesvalidarInvitacionAsistido");
        }

        Log.d(log, "+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++");

        //Prueba 2: El id_evento no existe en la colección
        Log.d(log + "-2", "Prueba de que no esta registrada un evento con ese id");

        exitosa = db.DesvalidarInvitacionAsistido(id_invitado, "54");
        if(exitosa) {//Se ha establecido la consulta con exito
            Log.d(log + "-2", "INCORRECTO---Existe el evento con id_evento");
        } else {
            Log.d(log + "-2", "CORRECTO---El evento con id_evento no existe");
            p_correcta++;
        }

        Log.d(log, "+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++");

        //Prueba 3: El id_ivitado no existe en la colección de invitados
        Log.d(log + "-3", "Prueba de que no esta registrada un invitado con ese id");

        exitosa = db.DesvalidarInvitacionAsistido("54", id_evento);
        if(exitosa) {//Se ha establecido la consulta con exito
            Log.d(log + "-3", "INCORRECTO---Existe el invitado con id_invitado");
        } else {
            Log.d(log + "-3", "CORRECTO---El invitado con id_invitado no existe");
            p_correcta++;
        }

        Log.d(log, "+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++");

        //Prueba 4: El id_ivitado e id_evento existen en la Base de Datos
        Log.d(log + "-4", "Prueba de que existe el id_ivitado e id_evento en la Base de Datos");

        exitosa = db.DesvalidarInvitacionAsistido(id_invitado, id_evento);
        if(exitosa) {//Se ha establecido la consulta con exito
            Log.d(log + "-4", "CORRECTO---Existe el invitado y evento, se ha quitado la asistencia al invitado");
            p_correcta++;
        } else {
            Log.d(log + "-4", "INCORRECTO---El invitado o evento no existen, no se pudo desregistrar");
        }
    ...
    /**---------------------------------------------------------------------------------------------
     * Funciones de test obsoletas de versiones anterirores  |---------------------------------------------|
     * -------------------------------------------------------------------------------------------*/
